...
const USUARIOS = {
    ...
    "nutri": { nome: "Dra. Fernanda Costa", perfil: "nutri", senha: "1234", cor: "#2e7d32" },
...
function mkPaciente(id, nome, nasc, curso, periodo, matricula, turno, cpf, rg, emailInst, emailPes, tel, end, tipo = "Estudante") {
    return {
        ...
        agendamentos: [],
        prontuarios: { nupss:null, fisio:null, odonto:null, nai:null, nae:null, nap:null, nutri:null },
        evolucoes: { nupss:[], fisio:[], odonto:[], nai:[], nae:[], nap:[], nutri:[] },
        encaminhamentos: [],
        auditoria: [],
    };
}
...
const INICIAL_PACIENTES = {
    "P001": (() => {
        ...
        p.prontuarios.nutri = { formaChegada:"espontanea", quemEncaminhou:"", queixaPrincipal:"Ganho de peso", objetivoConsulta:"Controle de peso e hábitos alimentares", peso:"70", altura:"1.62", circunferencia:"85", alergias:"—", habitoIntestinal:"Normal", diagnostico:"Sobrepeso", plano:"Dieta balanceada com acompanhamento mensal", metas:"Perder 5kg em 3 meses" };
...
const ABAS_CONFIG = {
    ...
    nutri:         ["resumo","nutri","agendamentos"],
...
const NUCLEOS_VISAO = [
    { id: "nupss", label: "🏠 NUPSS – Serviço Social", icon: "🏠", cor: "#2A7F6F" },
    { id: "nae", label: "🎓 NAE – Atendimento", icon: "🎓", cor: "#b45309" },
    { id: "nai", label: "♿ NAI – Acessibilidade", icon: "♿", cor: "#6B3FA0" },
    { id: "nap", label: "🧠 NAPp – Apoio Psicopedagógico", icon: "🧠", cor: "#be185d" },
    { id: "odonto", label: "🦷 Odontologia", icon: "🦷", cor: "#0e7490" },
    { id: "fisio", label: "🦴 Fisioterapia", icon: "🦴", cor: "#1B5E8A" },
    { id: "nutri", label: "🥗 Nutrição", icon: "🥗", cor: "#f59e0b" },
];
...
function renderLista() {
    ...
    // Visão por Núcleo para Supervisor/Coordenação
    if (appState.usuario.perfil === "supervisor" && appState.nucleo === null) {
        const nucleosHtml = NUCLEOS_VISAO.map(nuc => {
            const pacientesCount = Object.values(appState.pacientes).filter(p => {
                if (nuc.id === "nupss") return p.prontuarios.nupss;
                if (nuc.id === "nae") return p.prontuarios.nae;
                if (nuc.id === "nai") return p.prontuarios.nai;
                if (nuc.id === "nap") return p.prontuarios.nap;
                if (nuc.id === "odonto") return p.prontuarios.odonto;
                if (nuc.id === "fisio") return p.prontuarios.fisio;
                return false;
            }).length;
            
            return `
                <div class="nucleus-card" onclick="selecionarNucleo('${nuc.id}')">
                    <div class="nucleus-icon">${nuc.icon}</div>
                    <div class="nucleus-label">${nuc.label.split(' –')[0]}</div>
                    <div class="nucleus-count">${pacientesCount} pacientes</div>
                </div>
            `;
        }).join('');
        ...
        return `
            <div class="topbar">
                <div class="topbar-brand">
                    <img src="img/Icone IntegraSocial.png" alt="Ícone" class="topbar-logo" />
                    <div class="topbar-title">Prontuário Integrado – Coordenação</div>
                </div>
                <div class="topbar-user">
                    ...
                    <div class="topbar-info">
                        ...
                        <div class="topbar-info-role">Coordenação</div>
                    </div>
                    <button class="topbar-logout" onclick="fazerLogout()">Sair</button>
                </div>
            </div>
            <div class="main-container">
                <div class="nucleus-section">
                    <h2>VISÃO POR NÚCLEO</h2>
                    <p class="nucleus-subtitle">${NUCLEOS_VISAO.length} núcleos</p>
                    <div class="nucleus-grid">
                        ${nucleosHtml}
                    </div>
                </div>
                <div class="list-header">
                    <div class="list-title-group">
                        ...
                        <div class="records-count">4 registros</div>
                    </div>
                </div>
                <div class="list-grid">
    ...
    // Lista de Pacientes de um Núcleo Selecionado (Supervisor)
    if (appState.usuario.perfil === "supervisor" && appState.nucleo !== null) {
        const nucleoData = NUCLEOS_VISAO.find(n => n.id === appState.nucleo);
        const nucleoLabel = nucleoData ? nucleoData.label.replace(' – ', ' – ') : appState.nucleo;
        ...
        const pids = Object.keys(appState.pacientes).filter(id => {
            ...
            const p = appState.pacientes[id];
            if (appState.nucleo === "nupss") return p.prontuarios.nupss;
            if (appState.nucleo === "nae") return p.prontuarios.nae;
            if (appState.nucleo === "nai") return p.prontuarios.nai;
            if (appState.nucleo === "nap") return p.prontuarios.nap;
            if (appState.nucleo === "odonto") return p.prontuarios.odonto;
            if (appState.nucleo === "fisio") return p.prontuarios.fisio;
            return false;
        });
        
        const listCount = pids.length;
        const contentHtml = pids.map(id => {
            const m = appState.pacientes[id].meta;
            const tipo = (m.tipo || "Estudante");
            const idade = m.nascimento ? calcIdade(m.nascimento) : "—";
            const displayId = m.id;
            const temProntuario = appState.pacientes[id].prontuarios[appState.nucleo] ? "✓ Prontuário" : "Sem prontuário";
            return `
                <div class="student-item" onclick="selecionarPaciente('${id}')">
                    ...
                    <span class="badge ${temProntuario.includes('✓') ? "badge-green" : "badge-laranja"}">${temProntuario}</span>
                </div>
            `;
        }).join('');
        ...
        return `
            <div class="topbar">
                <button class="topbar-back" onclick="voltarParaNucleo()">← Voltar</button>
                <div class="topbar-brand">
                    <img src="img/Icone IntegraSocial.png" alt="Ícone" class="topbar-logo" />
                    <div class="topbar-title">Prontuário Integrado – Coordenação</div>
                </div>
            ...
            <div class="main-container">
                <div class="list-header">
                    <div class="list-title-group">
                        <div class="list-title">${nucleoLabel}</div>
                        <div class="records-count">${listCount} pacientes</div>
                    </div>
                    <div class="list-controls">
                        <input type="text" id="search-input" class="search-input" placeholder="Buscar por nome..." value="${appState.busca}">
                    </div>
                </div>
                <div class="list-grid">
                    ${contentHtml}
...
function renderListaPacientes() {
    const pids = Object.keys(appState.pacientes);
    return pids.map(id => {
        const m = appState.pacientes[id].meta;
        const tipo = (m.tipo || "Estudante");
        const idade = m.nascimento ? calcIdade(m.nascimento) : "—";
        ...
        return `
            <div class="student-item" onclick="selecionarPaciente('${id}')">
                ...
                <span class="badge ${tipo === "Comunidade" ? "badge-roxo" : "badge-azul"}">${tipo}</span>
            </div>
        `;
    }).join('');
}
...
function renderProntuario() {
    ...
    let headerLabel = (appState.usuario.perfil === 'nupss' || appState.usuario.perfil === 'odontologia') ? 'Pacientes & Usuários' : 'IntegraSocial · Prontuário Integrado';
    let backLabel = (appState.usuario.perfil === 'nupss' || appState.usuario.perfil === 'odontologia') ? '← Pacientes' : (appState.usuario.perfil === 'supervisor' && appState.nucleo ? `← ${NUCLEOS_VISAO.find(n => n.id === appState.nucleo)?.label.split(' –')[0]}` : '← Voltar');
    
    // Para supervisor vindo da coordenação
    if (appState.usuario.perfil === 'supervisor' && !appState.nucleo) {
        backLabel = '← Coordenação';
    }
...
function fazerLogin() {
    ...
    if (usr && usr.senha === pass && usr.perfil === perfilSel) {
        ...
        appState.tela = "lista";
        appState.nucleo = null; // Resetar seleção de núcleo
        appState.busca = ""; // Resetar busca
        render();
    } else {
...
function selecionarPaciente(pid) {
    appState.pidSel = pid;
    appState.abaSel = (appState.usuario.perfil === "nupss" || appState.usuario.perfil === "odontologia") ? "resumo" : (appState.usuario.perfil === "supervisor" ? "resumo" : "identificacao");
...
function selecionarNucleo(nucleoId) {
    appState.nucleo = nucleoId;
    appState.tela = "lista";
    render();
}

function voltarParaNucleo() {
    appState.nucleo = null;
    appState.busca = "";
    render();
}
